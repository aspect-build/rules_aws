...
def _aws_repo_impl(repository_ctx):
    url = "https://github.com/someorg/someproject/releases/download/v{0}/aws-{1}.zip".format(
        repository_ctx.attr.aws_version,
        repository_ctx.attr.platform,
    )
    repository_ctx.download_and_extract(
        url = url,
        integrity = TOOL_VERSIONS[repository_ctx.attr.aws_version][repository_ctx.attr.platform],
    )
    build_content = """#Generated by aws/repositories.bzl
load("@aspect_rules_aws//aws:toolchain.bzl", "aws_toolchain")
aws_toolchain(name = "aws_toolchain", target_tool = select({
        "@bazel_tools//src/conditions:host_windows": "aws_tool.exe",
        "//conditions:default": "aws_tool",
    }),
)
"""

...
def _install_linux(rctx, release_info):
    ...
    return (dist_dir, "//installed:{}/aws".format(dist_dir))
...
def _aws_repo_impl(rctx):
    if rctx.attr.platform.startswith("linux"):
        (version_dir, target_tool) = _install_linux(rctx, _release_info(rctx))
    elif rctx.attr.platform == "darwin":
        (version_dir, target_tool) = _install_darwin(rctx, _release_info(rctx))
...
aws_toolchain(name = "aws_toolchain", target_tool = "{}")
""".format(target_tool)
